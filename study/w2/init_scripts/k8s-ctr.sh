...
function parse_kubernetse_version() {
  local k8s_version=$1
  # if $1 contain '-' then return 1.33.2 else return 1.33.2-1.1
  if [ -z "$1" ]; then
    echo "Error: No version provided"
    return 1
  fi
  
  # if contain 'v' remove it
  if [[ $k8s_version == *"v"* ]]; then
    k8s_version=$(echo $k8s_version | cut -d 'v' -f2)
  fi

  if [[ $k8s_version == *"-"* ]]; then
    k8s_version=$(echo $k8s_version | cut -d '-' -f1)
  fi
...
curl --silent -o /root/kubeadm-init-ctr-config.yaml https://raw.githubusercontent.com/iamjjanga-ouo/cilium-lab/refs/heads/main/study/w2/kubeadm-configs/kubeadm-init-ctr-config.yaml
sed -i "s/K8S_VERSION_PLACEHOLDER/$(parse_kubernetse_version $3)/g" /root/kubeadm-init-ctr-config.yaml
